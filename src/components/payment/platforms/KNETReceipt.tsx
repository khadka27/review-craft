import { TransactionData } from "@/types/payment";
import { KNETIcon } from "@/components/SocialMediaIcons";

export const KNETReceipt = ({ data }: { data: TransactionData }) => {
  const isSuccess = data.status === "success";
  const isFailed = data.status === "failed";
  const formattedAmount = parseFloat(data.amount || "0").toLocaleString("en-US", {
    minimumFractionDigits: 3,
    maximumFractionDigits: 3,
  });

  return (
    <div
      style={{
        width: "100%",
        minHeight: "100%",
        backgroundColor: "#F8FAFC",
        fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif",
        display: "flex",
        flexDirection: "column",
        color: "#0F172A",
      }}
    >
      {/* ── KNET Kuwait Green Header ── */}
      <div
        style={{
          background: "#007A3D",
          padding: "32px 18px 14px",
          display: "flex",
          alignItems: "center",
          justifyContent: "space-between",
          color: "white",
        }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 30, height: 30, background: "white", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <KNETIcon size={20} />
          </div>
          <div>
            <span style={{ fontSize: 18, fontWeight: 900, color: "white", letterSpacing: 0.5 }}>
              KNET
            </span>
            <span style={{ fontSize: 11, color: "#BBF7D0", marginLeft: 6, fontWeight: 700 }}>
              Kuwait · الكويت
            </span>
          </div>
        </div>
        <span style={{ fontSize: 11, fontWeight: 800, background: "rgba(255,255,255,0.2)", color: "white", padding: "3px 10px", borderRadius: 12 }}>
          KWD د.ك
        </span>
      </div>

      {/* ── Content ── */}
      <div style={{ flex: 1, padding: "22px 16px 16px", display: "flex", flexDirection: "column" }}>
        <div
          style={{
            background: "white",
            borderRadius: 20,
            padding: "24px 18px",
            boxShadow: "0 4px 18px rgba(0, 122, 61, 0.08)",
            border: "1px solid #E2E8F0",
            textAlign: "center",
            marginBottom: 16,
          }}
        >
          <div
            style={{
              width: 68,
              height: 68,
              borderRadius: "50%",
              background: isSuccess ? "#ECFDF5" : isFailed ? "#FEF2F2" : "#FFFBEB",
              border: `3px solid ${isSuccess ? "#007A3D" : isFailed ? "#EF4444" : "#F59E0B"}`,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              margin: "0 auto 14px",
            }}
          >
            {isSuccess ? (
              <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#007A3D" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                <polyline points="20 6 9 17 4 12" />
              </svg>
            ) : isFailed ? (
              <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#EF4444" strokeWidth="3" strokeLinecap="round">
                <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
              </svg>
            ) : (
              <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#F59E0B" strokeWidth="3">
                <circle cx="12" cy="12" r="10" />
              </svg>
            )}
          </div>

          <p style={{ margin: "0 0 2px", fontSize: 14, color: "#64748B", fontWeight: 700 }}>
            {isSuccess ? "تمت عملية الدفع بنجاح" : isFailed ? "فشلت عملية الدفع" : "قيد المعالجة"}
          </p>
          <p style={{ margin: "0 0 10px", fontSize: 12, color: "#94A3B8" }}>
            {isSuccess ? "Transaction Successful" : "Transaction Failed"}
          </p>
          <h2 style={{ margin: "0 0 12px", fontSize: 20, fontWeight: 800, color: "#0F172A" }}>
            {data.receiverName}
          </h2>

          <div style={{ fontSize: 40, fontWeight: 900, color: "#007A3D", letterSpacing: -1.5, marginBottom: 8 }}>
            {formattedAmount} <span style={{ fontSize: 18, color: "#0F172A", fontWeight: 800 }}>KWD</span>
          </div>

          <div
            style={{
              display: "inline-flex",
              alignItems: "center",
              gap: 6,
              padding: "4px 14px",
              borderRadius: 16,
              background: "#ECFDF5",
              border: "1px solid #A7F3D0",
            }}
          >
            <span style={{ fontSize: 12, fontWeight: 800, color: "#065F46" }}>
              KNET Gateway · معتمد
            </span>
          </div>
        </div>

        {/* Details Card */}
        <div
          style={{
            background: "white",
            borderRadius: 18,
            padding: "16px 18px",
            border: "1px solid #E2E8F0",
            boxShadow: "0 2px 8px rgba(0,0,0,0.03)",
            marginBottom: 16,
          }}
        >
          {[
            { label: "المستفيد / Merchant", value: data.receiverName },
            { label: "الدافع / Customer", value: data.senderName },
            { label: "التاريخ / Date", value: data.timestamp },
            { label: "رقم المرجع / Ref No", value: `KN-${data.transactionId.slice(0, 10).toUpperCase()}`, mono: true },
            { label: "رمز الموافقة / Auth", value: data.transactionId.slice(0, 6).toUpperCase(), mono: true },
            ...(data.note ? [{ label: "الوصف / Note", value: data.note }] : []),
          ].map(({ label, value, mono }: any, i, arr) => (
            <div
              key={label}
              style={{
                display: "flex",
                justifyContent: "space-between",
                alignItems: "flex-start",
                paddingTop: i > 0 ? 11 : 0,
                paddingBottom: i < arr.length - 1 ? 11 : 0,
                borderBottom: i < arr.length - 1 ? "1px solid #F1F5F9" : "none",
              }}
            >
              <span style={{ fontSize: 12, color: "#64748B", fontWeight: 600 }}>{label}</span>
              <span
                style={{
                  fontSize: 13,
                  color: "#0F172A",
                  fontWeight: 700,
                  fontFamily: mono ? "monospace" : "inherit",
                  textAlign: "right",
                  maxWidth: "58%",
                }}
              >
                {value}
              </span>
            </div>
          ))}
        </div>
      </div>

      {/* ── Footer ── */}
      <div style={{ padding: "12px 16px 20px", background: "white", borderTop: "1px solid #E2E8F0" }}>
        <button
          style={{
            width: "100%",
            padding: "14px",
            background: "#007A3D",
            color: "white",
            border: "none",
            borderRadius: 26,
            fontSize: 15,
            fontWeight: 800,
            cursor: "pointer",
            boxShadow: "0 4px 14px rgba(0, 122, 61, 0.3)",
          }}
        >
          طباعة الإيصال / Print Receipt
        </button>
      </div>
    </div>
  );
};
